...
export const create_user = async ({ first_name, last_name, email, password }: { first_name: string, last_name: string, email: string, password: string }) => {
...
export const create_doctor = async ({ first_name, last_name, email, password }: { first_name: string, last_name: string, email: string, password: string }) => {
    ...
    const hash = await bcrypt.hash(password, saltRounds)
    const user = await publicRepository.create_user(value.first_name, value.last_name, value.email, hash, Role.DOCTOR)
